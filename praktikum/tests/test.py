...
def test_move_ingredient_invalid_index():
    burger = Burger()
    mock_ingredient = Mock()
    burger.add_ingredient(mock_ingredient)
    with pytest.raises(IndexError):
...
def test_move_ingredient_empty_list():
    burger = Burger()
    with pytest.raises(IndexError):
...
def test_get_receipt(bun_name, ingredient_data, expected_receipt):
    burger = Burger()
    mock_bun = Mock()
...
def test_get_bun_by_name_not_found():
    db = Database()
...
def test_get_ingredient_by_name_not_found():
    db = Database()
